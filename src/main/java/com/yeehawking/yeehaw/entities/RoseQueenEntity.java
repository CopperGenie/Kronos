...
public class RoseQueenEntity extends MonsterEntity {

    public RoseQueenEntity(EntityType<? extends MonsterEntity> type, World worldIn) {
        super(type, worldIn);
    }

    public static AttributeModifierMap.MutableAttribute setCustomAttributes() {
        return MobEntity.registerAttributes()
                .createMutableAttribute(Attributes.MAX_HEALTH, 285.0d)
    ...
    public boolean attackEntityAsMob(Entity entityIn) {
        ...
        if (flag && entityIn instanceof LivingEntity && rand.nextInt(5) == 0) {
            ...
            ((LivingEntity)entityIn).addPotionEffect(new EffectInstance(Effects.INSTANT_DAMAGE, 1, 0, false, true));
        }
        if (flag && entityIn instanceof LivingEntity) {
            float f = this.world.getDifficultyForLocation(this.getPosition()).getAdditionalDifficulty();
            ((LivingEntity)entityIn).addPotionEffect(new EffectInstance(Effects.POISON, 80 * (int)f, 1, false, true));
        }
