...
public class ToadFatherEntity extends MonsterEntity {

    public ToadFatherEntity(EntityType<? extends MonsterEntity> type, World worldIn) {
        super(type, worldIn);
    }
    ...
    public boolean attackEntityAsMob(Entity entityIn) {
        ...
        if (flag && entityIn instanceof LivingEntity) {
            ...
            ((LivingEntity)entityIn).addPotionEffect(new EffectInstance(Effects.NAUSEA, 100 * (int)f, 0, false, true));
        }
        if (flag && entityIn instanceof LivingEntity && rand.nextInt(2) == 0) {
            float f = this.world.getDifficultyForLocation(this.getPosition()).getAdditionalDifficulty();
            ((LivingEntity)entityIn).addPotionEffect(new EffectInstance(Effects.SLOWNESS, 80 * (int)f, 0, false, true));
        }
        if (flag && entityIn instanceof LivingEntity && rand.nextInt(2) == 0) {
            float f = this.world.getDifficultyForLocation(this.getPosition()).getAdditionalDifficulty();
            ((LivingEntity)entityIn).addPotionEffect(new EffectInstance(Effects.WEAKNESS, 80 * (int)f, 0, false, true));
        }
        if (flag && entityIn instanceof LivingEntity && rand.nextInt(2) == 0) {
            float f = this.world.getDifficultyForLocation(this.getPosition()).getAdditionalDifficulty();
