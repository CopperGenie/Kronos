...
public class FlintMongerEntity extends AnimalEntity {

    public static final Ingredient TEMPTATION_ITEMS = Ingredient.fromItems(Items.FLINT);

    public FlintMongerEntity(EntityType<? extends AnimalEntity> type, World worldIn) {
        super(type, worldIn);
    }

    public static AttributeModifierMap.MutableAttribute setCustomAttributes() {
        return MobEntity.registerAttributes()
                .createMutableAttribute(Attributes.MAX_HEALTH, 33.0d)
    ...
    public void livingTick() {
        if (this.world.isRemote) {
            ...
            if (rand.nextInt(20) == 0) {
                this.world.addParticle(ParticleTypes.SMOKE, this.getPosXRandom(0.5D), this.getPosYRandom() - 0.7, this.getPosZRandom(0.5D), 0.0D, -0.2D, 0.0D);
            }
        }
