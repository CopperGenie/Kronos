...
public class MinnasScroll extends Item {
    public MinnasScroll() {
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
        ...
        if (player.getHeldItemMainhand().getItem() == ModItems.MINNAS_SCROLL.get()) {
            ...
            for (int k = 1; k <= 4; k++) {
                ...
                if (world.getBlockState(player.getPosition().add(x, 0, z)) == Blocks.AIR.getDefaultState()) {
                    world.setBlockState(player.getPosition().add(x, 0, z), Blocks.ROSE_BUSH.getDefaultState());
                }
            }
