...
public class ShadowHorn extends Item {
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
        ...
        if (worldIn.getBlockState(new BlockPos(posX, posY, posZ)) == Blocks.BEDROCK.getDefaultState()) {
            if (!playerIn.getEntityWorld().isRemote) { playerIn.sendMessage(new StringTextComponent("Shadow Demon can't find you here!"), playerIn.getUniqueID()); }
        } else {
            if (worldIn instanceof ServerWorld) {
                ServerWorld serverWorld = (ServerWorld) playerIn.getEntityWorld();
                ModEntityType.SHADOW_DEMON.get().spawn(serverWorld, null, null, new BlockPos(posX,posY,posZ), SpawnReason.EVENT, false, true);
