...
public class UnchargedPhylactery extends Item {
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
        ...
        if (worldIn.getBlockState(new BlockPos(posX, posY, posZ)) == Blocks.BEDROCK.getDefaultState()) {
            if (!playerIn.getEntityWorld().isRemote) { playerIn.sendMessage(new StringTextComponent("Lich can't find you here!"), playerIn.getUniqueID()); }
        } else {
            if (worldIn instanceof ServerWorld) {
                ServerWorld serverWorld = (ServerWorld) playerIn.getEntityWorld();
                ModEntityType.LICH.get().spawn(serverWorld, null, null, new BlockPos(posX,posY,posZ), SpawnReason.EVENT, false, true);
