...
public class WorldKeyTheosUnderworld extends Item {
    public WorldKeyTheosUnderworld() {
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
        if (playerIn.getEntityWorld() instanceof ServerWorld) {
            ...
            if (playerIn.getHealth() >= 19.5) {
                if (playerIn.world.getDimensionKey().equals(Yeehaw.THEOS_UNDERWORLD)) {
                    targetWorld = serverPlayer.server.getWorld(RegistryKey.getOrCreateKey(Registry.WORLD_KEY,
                            new ResourceLocation("minecraft:overworld")));
                } else {
                    targetWorld = serverPlayer.server.getWorld(Yeehaw.THEOS_UNDERWORLD);
                }
