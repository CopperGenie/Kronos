...
public class ScrollOfFreezing extends Item {
    public ScrollOfFreezing() { super(new Properties().group(Yeehaw.SCROLL_TAB)); }
            ...
            if (rayTraceResult.getType() == RayTraceResult.Type.ENTITY) {
                ...
                if (!mobEntityList.isEmpty()) {
                    for (MobEntity entity : mobEntityList) {
                        // Add effects
                        entity.addPotionEffect(new EffectInstance(Effects.SLOWNESS, 20*8, 1, false, false));
                        if (entity.isEntityUndead()) {
                            entity.addPotionEffect(new EffectInstance(Effects.INSTANT_HEALTH, 1, 0, false, false));
                        } else {
