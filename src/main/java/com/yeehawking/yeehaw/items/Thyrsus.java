...
public class Thyrsus extends Item {
    public Thyrsus() {
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
        ...
        if (worldIn.getBlockState(new BlockPos(posX, posY, posZ)) == Blocks.BEDROCK.getDefaultState()) {
            if (!playerIn.getEntityWorld().isRemote) { playerIn.sendMessage(new StringTextComponent("Dionysus can't find you here!"), playerIn.getUniqueID()); }
        } else {
            if (worldIn instanceof ServerWorld) {
                ServerWorld serverWorld = (ServerWorld) playerIn.getEntityWorld();
                ModEntityType.DIONYSUS.get().spawn(serverWorld, null, null, new BlockPos(posX,posY,posZ), SpawnReason.EVENT, false, true);

                ItemStack itemstack = playerIn.getHeldItemMainhand();
                itemstack.shrink(1);
            }

            playerIn.playSound(SoundInit.LICH_SUMMON.get(), 1f, 0.8f);
        }
