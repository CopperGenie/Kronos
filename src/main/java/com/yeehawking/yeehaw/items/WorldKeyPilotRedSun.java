...
public class WorldKeyPilotRedSun extends Item {
    public WorldKeyPilotRedSun() {
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
        if (playerIn.getEntityWorld() instanceof ServerWorld) {
            ...
            if (playerIn.getHealth() >= 19.5) {
                if (playerIn.world.getDimensionKey().equals(Yeehaw.PILOT_RED_SUN)) {
                    targetWorld = serverPlayer.server.getWorld(RegistryKey.getOrCreateKey(Registry.WORLD_KEY,
                            new ResourceLocation("minecraft:overworld")));
                }

                else {
                    targetWorld = serverPlayer.server.getWorld(Yeehaw.PILOT_RED_SUN);
                }
        ...
        playerIn.playSound(SoundInit.GASMAN_HURT.get(), 0.7f, 1f);
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        ...
        tooltip.add(this.text1().mergeStyle(TextFormatting.RED));
    }
    // References en_us
    @OnlyIn(Dist.CLIENT)
